		...
		calculate : function(options){
			return this.each(function(){
				...
				if(typeof(options)!="undefined")
				{
					...
					$(this).html((options.currencyBefore!="" ? '<span class="currency">' + options.currencyBefore + '</span>' : '')+sum.toFixed(parseInt(options.decimalPlaces, 10)).replace(".", options.decimalSeparator).replace(regex, '$&' + options.thousandthSeparator)+(options.currencyAfter!="" ? '<span class="currency">' + options.currencyAfter + '</span>' : ''));
					if(jQuery.type(options.updateHidden)=="object")
					{
						options.updateHidden.val((options.currencyBefore!="" ? options.currencyBefore : '')+sum.toFixed(parseInt(options.decimalPlaces, 10)).replace(".", options.decimalSeparator).replace(regex, '$&' + options.thousandthSeparator) + (options.currencyAfter!="" ? options.currencyAfter : ''));
