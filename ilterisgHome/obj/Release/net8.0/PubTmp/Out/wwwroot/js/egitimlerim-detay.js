document.addEventListener("DOMContentLoaded", function () {
    ...
    if (startExamBtn) {
        startExamBtn.addEventListener("click", function () {
            // Sınavı sıfırlamak için bir onay mesajı göster
            Swal.fire({
                icon: 'warning',
                title: 'Sınava Yeniden Başlamak İstiyor musunuz?',
                text: 'Eğer sınavı yeniden başlatırsanız, önceki sınav sonuçlarınız sıfırlanabilir.',
    ...
    if (approveDocumentBtn) {
        approveDocumentBtn.addEventListener("click", function () {
            ...
            // Canvas’ın arka planını manuel olarak beyaza boya
            ...
            signaturePad.clear();
            // Canvas’ın görünürlüğünü ve boyutlarını kontrol et
            console.log("Canvas genişlik:", canvas.width, "Yükseklik:", canvas.height);
            // Test çizimi: Canvas’a manuel bir çizgi çiz
            ctx.beginPath();
            ctx.moveTo(10, 10);
            ctx.lineTo(100, 100);
            ctx.strokeStyle = "red";
            ctx.lineWidth = 2;
            ctx.stroke();
            console.log("Test çizimi yapıldı: Kırmızı bir çizgi çizildi.");
        });
    }

    // Modal kapatıldığında odak yönetimini düzelt
    signatureModal._element.addEventListener('hidden.bs.modal', function () {
        // Odaklanmayı güvenli bir elemente geri yönlendir
        if (signDocumentBtn) {
            signDocumentBtn.focus();
        }
    });
    ...
    if (saveSignatureBtn) {
        saveSignatureBtn.addEventListener("click", function () {
            ...
            console.log("Çizim verisi:", signatureData); // Çizim verisini konsola yazdır
            if (!signatureData) {
            ...
            try {
                ...
                console.error("jsPDF addImage hatası:", err);
                Swal.fire('Hata!', 'İmza eklenirken bir hata oluştu: ' + err.message, 'error');
                return;
            }
