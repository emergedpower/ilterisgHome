...
jQuery(document).ready(function($){
	...
	$("#final-service-cost").costCalculator({
		formula: "sales-per-month-value*10000",
		currencyBefore: "₺",
		currencyAfter: "/ Ay",
		thousandthSeparator: ",",
		decimalSeparator: ".",
		decimalPlaces: 0,
		updateHidden: $("#final-service-cost-total") // Gizli input güncelleniyor
	});
	//home 2 slider
	...
	{
// Varsayılan koordinat değerleri
var defaultLat = 41.07079256581832;
var defaultLng = 28.805494983319694;

// Haritanın merkez koordinatını ayarla (data attribute varsa onu kullanır, yoksa varsayılanı)
var coordinate = new google.maps.LatLng(
    ($("#map").data("map-center-lat") ? $("#map").data("map-center-lat") : defaultLat),
    ($("#map").data("map-center-lng") ? $("#map").data("map-center-lng") : defaultLng)
);

// Harita seçenekleri (zoom değerini isteğe göre arttırabilirsiniz, burada 15 yapıldı)
var mapOptions = {
    zoom: ($("#map").data("zoom") ? $("#map").data("zoom") : 30),
    center: coordinate,
...
// Marker oluştururken hem data attribute varsa onu kullanır, yoksa varsayılan koordinatlar atanır.
var marker = new google.maps.Marker({
    position: new google.maps.LatLng(
        ($("#map").data("marker-lat") ? $("#map").data("marker-lat") : defaultLat),
        ($("#map").data("marker-lng") ? $("#map").data("marker-lng") : defaultLng)
    ),
    map: map,
