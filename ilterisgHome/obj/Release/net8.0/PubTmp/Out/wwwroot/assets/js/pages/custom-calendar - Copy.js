...
//    // Roller yüklenmeden önce fonksiyonları beklet
//    getUserRoles().then(() => {
...
//                    // OSGB kullanıcıları için firma seçimi modalını aç
//                    firmSelectionModal.show();
//                } else {
//                    // OSGB dışındaki kullanıcılar için doğrudan kişisel etkinlik modalını aç
//                    newPersonalEtkinlik(new Date().toISOString().split("T")[0], 'Etkinlik');
//                }
//            });
//        }
...
//            newEgitim(personalEventForm.dataset.date || new Date().toISOString().split("T")[0]);
//        });

//        document.getElementById('select-etkinlik').addEventListener('click', function () {
//            eventTypeSelectionModal.hide();
//            newPersonalEtkinlik(personalEventForm.dataset.date || new Date().toISOString().split("T")[0], 'Etkinlik');
//        });

//        document.getElementById('select-toplanti').addEventListener('click', function () {
//            eventTypeSelectionModal.hide();
//            newPersonalEtkinlik(personalEventForm.dataset.date || new Date().toISOString().split("T")[0], 'Toplanti');
//        });

//        document.getElementById('select-ziyaret').addEventListener('click', function () {
//            eventTypeSelectionModal.hide();
//            newPersonalEtkinlik(personalEventForm.dataset.date || new Date().toISOString().split("T")[0], 'Ziyaret');
//        });

//        document.getElementById('select-diger').addEventListener('click', function () {
//            eventTypeSelectionModal.hide();
//            newPersonalEtkinlik(personalEventForm.dataset.date || new Date().toISOString().split("T")[0], 'Diger');
//        });
...
//                // Yeni etkinlik oluştururken selectedEvent null olmalı
//                selectedEvent = null;

//                const firmaId = info.event.extendedProps.firmaId;
//                const start = new Date(info.event.start);
//                const end = calendar.view.type === 'dayGridMonth' ? null : new Date(start.getTime() + (info.event.extendedProps.sure || 1) * 60 * 60 * 1000);

//                eventForm.dataset.date = info.event.startStr;
//                eventForm.dataset.event = JSON.stringify({ id: null, title: info.event.title });
//                eventFirmaId.value = firmaId;

//                // Modal açılmadan önce butonları gizle
...
//                    extendedProps: {
//                        firmaId: firmaId,
//                        sure: info.event.extendedProps.sure || 1,
//                        tehlikeSinifi: "AzTehlikeli",
//                        egitimTuruId: null,
//                        isTemp: true,
//                        type: 'egitim'
//                    }
...
//                if (info.event) {
//                    info.event.remove(); // Orijinal draggable’ı kaldır
//                }
//            },
...
//                        loadOSGBFirms();
//                        if (selectedEvent.extendedProps.firmaId) loadPersoneller(selectedEvent.extendedProps.firmaId);
...
//                if (userRoles.includes('OSGB')) {
//                    firmSelectionModal.show();
//                    eventForm.dataset.date = info.dateStr;
//                    eventForm.dataset.event = JSON.stringify({ id: null, title: "Yeni Eğitim" });
//                    personalEventForm.dataset.date = info.dateStr;
//                    personalEventForm.dataset.event = JSON.stringify({ id: null, title: "Yeni Etkinlik" });
//                    eventFirmaId.value = "";
//                } else {
//                    newPersonalEtkinlik(info.dateStr, 'Etkinlik');
//                }
//            }
//        });
...
//                        });
//                    } else {
//                        Swal.fire('Hata!', 'Firmalar yüklenemedi.', 'error');
//                    }
//                })
//                .catch(err => {
...
//        function loadPersoneller(firmaId) {
//            fetch(`/api/Calendar/GetFirmPersoneller?firmaId=${firmaId}`, {
//                method: 'GET',
//                headers: { 'Accept': 'application/json', 'Content-Type': 'application/json' }
//            })
//                .then(response => {
...
//                        });
//                        console.log("Personeller yüklendi:", data.data.length);
//                    } else {
//                        console.warn("Personel bulunamadı:", data.message);
//                        Swal.fire('Uyarı!', 'Personel bulunamadı.', 'warning');
//                    }
//                })
//                .catch(err => {
...
//                personelSection.style.display = 'block';
//                loadOSGBFirms();
//                if (selectedFirmaId) loadPersoneller(selectedFirmaId);
//            } else {
//                firmaSection.style.display = 'none';
//                personelSection.style.display = 'none';
//            }
